...
const WorldCupPostCommentsBox = ({ postId, winnerTitle }) => {
  ...
  const handleCommentSubmit = async () => {
    if (comment.trim()) {
      const isCreated = await createComment(`(${winnerTitle}) ` + comment, postId);
      if (isCreated) {
        setComment("");
        fetchComments();
      }
  ...
  const handleResponseCommentSubmit = async () => {
    if (comment.trim() && isResponseCommentOn && parentCommentId !== null) {
      const isCreated = await createResponseComment(`(${winnerTitle}) ` + comment, parentCommentId);
      if (isCreated) {
        setComment("");
        fetchComments();
...
export default WorldCupPostCommentsBox;